...
import { getBestOfLiveEvents } from "../redux/actions/moviesActions";
import { ChevronRight } from "lucide-react";

export const BestOfLiveEvents = () => {
  const dispatch = useDispatch();
  const { bestOfLiveEventsLoading, bestOfLiveEvents, bestOfLiveEventsError } = useSelector(
    (state) => state.movieData
  );

  useEffect(() => {
    dispatch(getBestOfLiveEvents());
  }, [dispatch]);

  return (
    <div className="container mx-auto py-8 px-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-textLight">Best of Live Events</h2>
        <a href="/activities" className="text-primary flex items-center group">
          See All
          <ChevronRight className="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform duration-200" />
        </a>
      </div>

      {bestOfLiveEventsLoading ? (
        <p>Loading best of live events...</p>
      ) : bestOfLiveEventsError ? (
        <p className="text-red-500">Error: {typeof bestOfLiveEventsError === 'object' ? (bestOfLiveEventsError.error || JSON.stringify(bestOfLiveEventsError)) : bestOfLiveEventsError}</p>
      ) : (
        <div className="flex overflow-x-scroll no-scrollbar space-x-4 pb-4">
          {bestOfLiveEvents.map((event) => (
            <div key={event.eventId} className="flex-none w-64 bg-white dark:bg-footerBg shadow-md rounded-lg overflow-hidden relative cursor-pointer transform transition-transform duration-300 hover:scale-105">
              <div className="relative">
                <img src={event.posterUrl} alt={event.title} className="w-full h-80 object-cover rounded-t-lg" />
                {/* Overlay for rating and votes - adapted for events if needed */}
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/70 to-transparent p-4 text-white">
                  {event.rating && event.votes && (
                    <p className="text-base font-semibold flex items-center">
                      ...
                      {event.rating}/10 {event.votes} Votes
                    </p>
                  )}
                  {event.likes && !event.rating && !event.votes && (
                    <p className="text-base font-semibold flex items-center">
              ...
              <div className="px-3 py-2">
                <h4 className="text-lg font-bold dark:text-textLight">{event.title}</h4>
                <p className="text-sm text-gray-500 dark:text-footerText mb-1">{event.genre}</p>
              </div>
            </div>
          ))}
        </div>
